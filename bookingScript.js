...
function preFilledForm() {
  ...
  formBN.value =
    '=VALUE(CONCATENATE(RIGHT(YEAR(INDIRECT("E" & ROW())),2), TEXT(MONTH(INDIRECT("E" & ROW())), "00"), TEXT(COUNTIF(INDIRECT("O1:O" & ROW()), INDIRECT("O" & ROW())), "00")))';
}
  ...
  fetch("https://docs.google.com/spreadsheets/d/e/2PACX-1vRilpgg9SMxdf64sZqm5n2bd8czDHseC2bQAkvfaHRw46wX8hC00Ys_ihBNvNZC8-_pH9oaCmr5cKMk/pub?gid=923930940&single=true&output=csv"
    ...
    .then((sCsv) => {
      ...
      sData.forEach((entry) => {
        ...
        //status update
        sCard.children[1].children[0].innerHTML =
          entry["Status"] === "" ? "New Booking" : entry["Status"];
        ...
        if (entry["Vehicle Number"] != "") {
          sCard.children[2].children[10].innerHTML = `Vehicle Number: ${entry[
            "Vehicle Number"
          ].toUpperCase()}, ${entry["Kilometer"]} Kilometer`;
        }
        // mechanics
...
let bookingStatusForm =
  document.getElementById("bookingStatus").children[1].children[0].children;
